...
const DropZoneContent: React.FC<{ maxSize: number; limit: number }> = ({
  ...
  <>
    ...
    <div className="tiptap-image-upload-content">
      <span className="tiptap-image-upload-text">
        <em>{editorT("editor.upload_click")}</em> {editorT("editor.upload_drag")}
      </span>
      <span className="tiptap-image-upload-subtext">
        {editorT("editor.upload_max")} {limit} · {maxSize / 1024 / 1024}MB
        each.
      </span>
    </div>
  </>
        ...
        <div className="tiptap-image-upload-previews">
          {fileItems.length > 1 && (
            <div className="tiptap-image-upload-header">
              <span>{editorT("editor.upload_uploading")} {fileItems.length}</span>
              <Button
                type="button"
                data-style="ghost"
                onClick={(e) => {
              ...
              >
                {editorT("editor.upload_clear_all")}
              </Button>
            </div>
          )}
