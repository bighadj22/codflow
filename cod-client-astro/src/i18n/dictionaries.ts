...
export const NAMESPACES = ["orders", "auth", "common", "navigation", "dashboard", "customers", "customer-groups", "customer-tags", "reviews", "products", "product-groups", "offers", "delivery", "delivery_companies", "settings", "team", "mcp", "profile"] as const;
export type Namespace = (typeof NAMESPACES)[number];
export type Dict = Record<string, unknown>;

const DICTS: Record<Locale, Partial<Record<Namespace, Dict>>> = {
  ar: { orders: arOrders, auth: arAuth, common: arCommon, navigation: arNavigation, dashboard: arDashboard, customers: arCustomers, "customer-groups": arCustomerGroups, "customer-tags": arCustomerTags, reviews: arReviews, products: arProducts, "product-groups": arProductGroups, offers: arOffers, delivery: arDelivery, delivery_companies: arDeliveryCompanies, settings: arSettings, team: arTeam, mcp: arMcp, profile: arProfile },
